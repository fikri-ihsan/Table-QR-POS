    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {showForm && (
        <div className="bg-white border border-zinc-200 rounded-2xl p-6 mb-6 flex gap-4 items-end">
          <div>
            <label className="block text-sm font-medium text-zinc-700 mb-1">Nomor Meja</label>
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-700 mb-1">Kapasitas</label>
          ...
          <div key={table.id} className="bg-white border border-zinc-200 rounded-2xl p-5 space-y-3">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-xs text-zinc-500">Kapasitas {table.capacity} orang</p>
              </div>
              <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                table.status === "available" ? "bg-green-100 text-green-700" : "bg-zinc-100 text-zinc-600"
              }`}>
            ...
            {qrMap[table.id] ? (
              <div className="text-xs bg-zinc-50 rounded-xl p-3 break-all font-mono text-zinc-500">
                {qrMap[table.id]}
              </div>
            ) : (
              <button
                onClick={() => generateQR(table.id)}
                className="w-full py-2 rounded-xl border border-dashed border-zinc-300 text-xs text-zinc-500 hover:bg-zinc-50"
              >
                Generate QR Code
              </button>
            )}
            ...
            <div className="flex justify-between text-xs pt-2 border-t border-zinc-100">
              <span className="text-zinc-400">ID: {table.id.slice(0, 8)}...</span>
